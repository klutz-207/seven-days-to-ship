...
《七天之后》— AI 叙事驱动的 Hackathon 模拟器。玩家观察一个 AI 数字人在 7 天内完成项目，可以随时用文字干预其决策。数字人有自主性，会自己决定去哪个房间、做什么任务，玩家的干预会影响其方向。
...
app/
  ...
  api/
    character/          上帝推演接口（名字→性格）
    bubble/             房间气泡接口（思考生成）
    decision/           AI 决策接口（玩家干预→反应）
    journal/            DDAE 日志接口
    ending/             最终产品接口
    plan/               每日计划接口
components/
  NameInput.tsx         命名页面
  GodNarration.tsx      上帝推演（流式输出）
  CharacterCard.tsx     角色卡片展示
  StartScreen.tsx       开始页面
  CorridorScene.tsx     走廊场景
  RoomStage.tsx         房间场景（三阶段：走到中间→等待→离开）
  CharacterSprite.tsx   角色精灵动画
  EventBubble.tsx       气泡组件
  DialogueDock.tsx      输入框（纯文本，无按钮）
  DailyNote.tsx         每日计划纸条
  JournalNote.tsx       DDAE 日志纸条
...
lib/
  llm.ts                统一 LLM 调用封装
  llmClient.ts          前端 API 调用封装
  conversation.ts       对话历史管理
  types.ts              TypeScript 类型定义
  rooms.ts              房间定义 + 事件池
  eventEngine.ts        事件触发 + 权重计算
...
| 房间 | 功能 | 核心收益 | 代价 |
|------|------|----------|------|
| 电脑桌 | 写代码、接 API | 功能+12, 稳定+3 | 压力+6, 注意力-5 |
| 书桌 | 策划、复盘 | 清晰+10, 创意+6 | 注意力-2 |
| 咖啡厅 | 交流、反馈 | 清晰+8, 展示+3 | 压力+4, 注意力-3 |
| 卧室 | 休息、恢复 | 压力-12, 自我感+5, 注意力+8 | 进度停滞 |
| 展台 | 演示、包装 | 展示+12, 清晰+4 | 压力+6, 稳定-1 |
